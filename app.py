...
def home():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Books WHERE Image_URL_S != '987654321'")
...
def admin_books():
    ...
    if search_query:
            ...
            cursor.execute("""
                ...
                FROM Books 
                WHERE (ISBN LIKE ? OR LOWER(Book_Title) LIKE ? OR LOWER(Book_Author) LIKE ?) AND Image_URL_S != '987654321'
...
def delete_book():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE Books SET Image_URL_S = '987654321' WHERE ISBN = ?", (isbn,))
    conn.commit()
    conn.close()
...
def admin_users():
    ...
    if search_query:
        ...
        cursor.execute("SELECT TOP(1000) User_ID, Location, Age FROM Users WHERE (Age != 987654321 OR Age IS NULL) AND (User_ID LIKE ? OR Location LIKE ?)", ('%' + search_query + '%', '%' + search_query + '%'))
    else:
        # Nếu không có tham số tìm kiếm, lấy tất cả người dùng
        cursor.execute("SELECT TOP(1000) User_ID, Location, Age FROM Users WHERE Age != 987654321 OR Age IS NULL")
...
def delete_user():
    ...
    cursor.execute("UPDATE Users SET Age = 987654321 WHERE User_ID = ?", (user_id,))
    conn.commit()
    conn.close()
